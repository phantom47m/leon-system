...
from .conversation_mixin import (
    ConversationMixin, _is_trivial_conversation,
    _TRIVIAL_EXACT, _TRIVIAL_CHAT_RE, _TRIVIAL_SUFFIX_RE, _TRIVIAL_PUNCT_RE,
)
...
# _is_trivial_conversation → conversation_mixin.py
...
class Leon(RoutingMixin, BrowserMixin, TaskMixin, AwarenessMixin, ReminderMixin, ResponseMixin, ConversationMixin):
    """Main orchestration system - the brain that coordinates everything.

    Routing, browser automation, task orchestration, awareness loop, reminders,
    response formatting, and conversation processing are implemented in their
    respective mixin files (core/*_mixin.py).
    """
    ...
    # _extract_memory            → conversation_mixin.py
    # _check_sensitive_permissions → conversation_mixin.py
    # _analyze_request            → conversation_mixin.py
    # _respond_conversationally   → conversation_mixin.py
